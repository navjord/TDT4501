...
double MedLDA::compute_lhood(Document* doc, double** phi, double* var_gamma)
{
	...
	lhood = boost::math::lgamma( alpha_sum ) - (boost::math::lgamma(var_gamma_sum));
	for ( int k=0; k<m_nK; k++ ) {
		lhood -= boost::math::lgamma( m_alpha[k] );
	}

	for (int k = 0; k < m_nK; k++) {
		lhood += (m_alpha[k] - 1)*(dig[k] - digsum) + boost::math::lgamma(var_gamma[k])
...
int MedLDA::run_em(char* start, char* directory, Corpus* corpus, Params *param)
{
	...
	int i = 0;
	double lhood, obj_old = 0, converged = 1;
	int nIt = 0;
		...
		|| (i <= 2)) && (i <= param->EM_MAX_ITER))
	{
		...
		double dobj = lhood + m_dsvm_primalobj;

		converged = (obj_old - dobj) / (obj_old);
		//if (converged < 0) VAR_MAX_ITER = VAR_MAX_ITER * 2;
		obj_old = dobj;

		// output model and lhood
		fprintf(likelihood_file, "%10.10f\t%10.10f\t%5.5e\n", lhood, dobj, converged);
...
SAMPLE MedLDA::trans_svm_format(SuffStats* ss, STRUCT_LEARN_PARM *sparm)
{
	SAMPLE sample;
	sample.examples = (EXAMPLE *)my_malloc(sizeof(EXAMPLE) * ss->num_docs);

	TOKEN *words = (TOKEN *)my_malloc(sizeof(TOKEN) * (m_nK + 10));
	int totwords = m_nK;
	for ( int d=0; d<ss->num_docs; d++ ) {
		...
		for ( int k=0; k<m_nK; k++ ) {
			...
			(words[k]).wnum = wnum;
			(words[k]).weight = ss->exp[d][k];
		}
		(words[m_nK]).wnum=0;

		// set the sample.
		sample.examples[d].x.doc = create_example(d, 0, 0, 1, create_svector(words, NULL, 1.0));;
		sample.examples[d].y.classlabel = ss->y[d] + 1;
		sample.examples[d].y.scores = NULL;
		sample.examples[d].y.num_classes = m_nLabelNum;
	}
	sample.n = ss->num_docs;
...
void MedLDA::svmStructSolver(SuffStats* ss, Params *param, double *res)
{
	...
	//SAMPLE sample  = trans_svm_format( ss, &struct_parm );
	//free_struct_sample(sample1);
	...
	*/
	...
	//		for ( int d=0; d<ss->num_docs; d++ ) {
	//			int muIx = d * m_nLabelNum + y;
	//			m_dEta[etaIx] -= m_dMu[muIx] * ss->exp[d][k];
	//		}
	//	}
	//}
	...
	//			dMu += m_dMu[muIx];
	//		}
	//		m_dEta[gndetaIx] += dMu * ss->exp[d][k];
	//	}
	//}
...
double MedLDA::infer(char* model_dir, Corpus* corpus, Params *param)
{
	...
	fclose(fileptr);
	long runtime_end = get_runtime();
	printf("Testing time in (cpu-seconds): %.2f\n", ((float)runtime_end-(float)runtime_start)/100.0);
	printf("Average doc-entropy: %.3f\n", dEntropy / corpus->num_docs);
