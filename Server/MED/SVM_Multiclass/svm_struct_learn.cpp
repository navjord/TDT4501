					  ...
					  STRUCTMODEL *sm, int alg_type)
{
	...
	if(alg_type == NSLACK_SHRINK_ALG) use_shrinking = 1;
	else use_shrinking = 0;
	...
	tolerance=MIN(n/3,MAX(n/100,5));/* increase precision, whenever less
									than that number of constraints	is not fulfilled */
	...
	if(kcache) kernel_cache_cleanup(kcache);
		...
		do { /* iteration until (approx) all SV are found for current
			...
			do { /* with shrinking turned on, go through examples that keep
				...
				for(i=0; i<n; i++) { /*** example loop ***/
					...
					{
						/* if the example is not shrunk	away, then see if it is necessary to 
						...
						/**** get psi(y)-psi(ybar) ****/
						rt2 = get_runtime();
						if(fycache) fy = copy_svector(fycache[i]);
						else fy = psi(ex[i].x,ex[i].y,sm,sparm);
						...
						if( sparm->slack_norm == 2 ) lossval = sqrt(lossval);
						if(sparm->loss_type == SLACK_RESCALING) factor = lossval;
				...
				rt_total += MAX(get_runtime()-rt1,0);
													   ...
													   produced no constraint at least once */
	...
	//if(struct_verbosity>=1) {
		...
		maximum slack id is larger than what is allocated below ****/
		...
		if(sparm->slack_norm == 1) {
			...
			for(j=0;j<cset.m;j++) 
				slacks[cset.lhs[j]->slackid] = MAX(slacks[cset.lhs[j]->slackid], cset.rhs[j]
												-(classify_example(svmModel,cset.lhs[j])
													-sm->w[sizePsi+cset.lhs[j]->slackid-1]/(sqrt(2*svmCnorm))));
		}
		slacksum = 0;
		for(i=1; i<=n; i++) slacksum += slacks[i];
		free(slacks);
		alphasum = 0;
		for(i=0; i<cset.m; i++) alphasum += alpha[i]*cset.rhs[i];
	...
	if(struct_verbosity>=4) printW(sm->w,sizePsi,n,lparm->svm_c);
	...
	/* record the original document ID in support vectors. */
	for ( int i=1; i<sm->svm_model->sv_num; i++ ) {
		long docnum = sm->svm_model->supvec[i]->docnum;
	...
	if(fycache) {
		for(i=0;i<n;i++) free_svector(fycache[i]);
		free(fycache);
	}
	if(svmModel) free_model(svmModel,0);
	free(alpha); 
	free(alphahist); 
	free(opti); 
	free(cset.rhs); 
	for(i=0;i<cset.m;i++) free_example(cset.lhs[i],1);
	free(cset.lhs);
}
	...
	*/
	...
	do { /* iteratively find and add constraints to working set */
		...
		if(alg_type == ONESLACK_DUAL_CACHE_ALG) {
			...
			if(viol-slack > MAX(epsilon_est/10,sparm->epsilon)) { 
				...
				for(j=0; (j<batch_size) || ((j<n)&&(viol-slack<sparm->epsilon)); j++) {
					...
					LABEL ybar = find_most_violated_constraint(&fydelta, &rhs_i, &ex[i],
			...
			for(i=0; i<n; i++) {
				...
				//if(struct_verbosity >= 1) print_percent_progress(&progress,n,10,".");
		...
		if( (ceps > sparm->epsilon) || cached_constraint ) { 
			...
			/* set the loss-augmented label for each document */
			cset.lhs[cset.m]->lvec = (int*)malloc(sizeof(int) * n);
			for ( int i=0; i<n; i++ ) {
			...
			svm_learn_optimization(cset.lhs, cset.rhs, cset.m, sizePsi, lparm, kparm, NULL, svmModel, alpha);
	...
	if(kparm->kernel_type == LINEAR) modellength = model_length_n(svmModel);
	...
	if(svmModel) {
		...
		for ( int i=1; i<svmModel->sv_num; i++ ) {
			sm->svm_model->supvec[i]->lvec = (int*)malloc(sizeof(int)*n);
			for ( int d=0; d<n; d++ ) {
...
//	print_struct_learning_stats(sample,sm,cset,alpha,sparm);
	...
	if(lhs_n) free_nvector(lhs_n);
	if(ccache) free_constraint_cache(ccache);
	for(i=0;i<n;i++)
		if(fycache[i]) free_svector(fycache[i]);
	free(fycache);
	free(alpha); 
	free(alphahist); 
	free(cset.rhs); 
...
LABEL find_most_violated_constraint(SVECTOR **fydelta, double *rhs, 
   ...
   to the most violated constraint for example ex */
{
	...
	if(struct_verbosity>=2) rt2 = get_runtime();
	if(fycached) fy = copy_svector(fycached); 
	else fy = psi(ex->x,ex->y,sm,sparm);
...
void remove_inactive_constraints(CONSTSET *cset, double *alpha, 
								 long currentiter, long *alphahist, long mininactive)
  /* removes the constraints from cset (and alpha) for which alphahist indicates that 
  they have not been active for at least mininactive iterations */
{  
	long i,m;
