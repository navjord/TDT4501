...
LABEL       find_most_violated_constraint_marginrescaling(PATTERNX x, LABEL y, 
						     STRUCTMODEL *sm, 
						     STRUCT_LEARN_PARM *sparm)
{
  /* Finds the label ybar for pattern x that that is responsible for
			       ...
			       STRUCT_LEARN_PARM *sparm)
{
  ...
  for(i=1;i<model->sv_num;i++) {
	  for(v=model->supvec[i]->fvec;v;v=v->next) {
		  ...
		  if(v->userdefined) fprintf(modelfl,"#%s\n",v->userdefined);
		  else fprintf(modelfl,"#\n");
